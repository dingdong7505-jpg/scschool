...
const MPMeetings=({meetings,setMeetings,sections})=>{
  ...
  const [showAdd,setShowAdd]=useState(false),[detail,setDetail]=useState(null),[filterSec,setFilterSec]=useState('all'),[editDoc,setEditDoc]=useState(null);
  ...
  const DocForm=({initial,onSave,onClose})=>{
    const [f,setF]=useState(initial||{title:'',date:todayStr(),uploader:'',category:tab==='lesson'?'공과':'회의록',sectionId:'all',content:'',fileName:'',fileData:''});
    const set=(k,v)=>setF(p=>({...p,[k]:v}));
    ...
    return <div className="space-y-3"><Inp label="제목" value={f.title} onChange={v=>set('title',v)} required/>
      ...
      <div className="flex gap-2 pt-1"><button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm">취소</button><button onClick={()=>{if(!f.title)return alert('제목 입력');onSave(f);onClose();}} className="flex-1 py-2.5 bg-[#1a1a1a] text-white rounded-xl text-sm">{initial?'수정 완료':'저장'}</button></div></div>;
  };
  const download=m=>{
